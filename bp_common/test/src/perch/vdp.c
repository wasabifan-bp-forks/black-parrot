...
void bp_call_zipline_accelerator(uint8_t type, struct Zipline_CSR zipline_csrs, uint64_t input_tlv_num)
{
  ...
  cfg_base_addr = (uint64_t *)(0x02100000);//device 1/compression engine in the tile
  cfg_base_dma_addr = (uint64_t *)(0x02200000);//dma device id
  bp_set_mmio_csr(cfg_base_addr, RESP_PTR, (uint64_t) zipline_csrs.resp_ptr);
  //send input
  int i=0;
  ...
  while (1)
    {
      status = bp_get_mmio_csr(cfg_base_addr, RESP_DONE);
      if(status)
        break;
    }
